...
def query_knowledge_base(tenant_id: str, customer_id: str, query: str) -> dict:
    ...
    try:
        ...
        # Format embedding as a string with proper PostgreSQL vector syntax (brackets instead of braces)
        embedding_vector = embedding_result["embedding"]
        embedding_str = '[' + ','.join([str(x) for x in embedding_vector]) + ']'
        logger.info(f"Using vector embedding format: {embedding_str[:50]}...")
        
        # SQL for vector similarity search with pgvector - using cast to vector
        sql_query = f"""
        ...
        try:
            ...
            logger.info(f"Query will search for {len(embedding_vector)} dimensional vectors")
