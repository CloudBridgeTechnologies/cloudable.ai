...
resource "aws_iam_role_policy" "agent" {
  ...
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      { Effect = "Allow", Action = ["bedrock:InvokeModel", "bedrock:InvokeModelWithResponseStream", "bedrock:Retrieve", "bedrock:InvokeAgent", "bedrock:GetAgent", "bedrock:GetAgentAlias"], Resource = "*" },
...
resource "aws_iam_role" "kb" {
  ...
  name               = "kb-role-${var.env}-${each.value.name}"
  assume_role_policy = data.aws_iam_policy_document.assume_bedrock.json
