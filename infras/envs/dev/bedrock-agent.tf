resource "aws_bedrockagent_agent" "tenant" {
  ...
  instruction             = "You are the ${each.value.name} assistant. Allowed intents: journey.status, assessment.summary. Refuse all else."
...
paths:
  /invoke:
    post:
      description: "Invoke DB read via Lambda with tenant and customer."
      operationId: invoke_db_read
      requestBody:
        required: true
        content:
          application/json:
            ...
            application/json:
              schema:
                ...
                properties:
                  result:
                    type: string
                    description: "Serialized result or summary"
EOF
  }
