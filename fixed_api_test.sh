...
# Fixed API Test Script for Cloudable.AI
# Tests all APIs with correct request formats based on Lambda handler requirements
...
# 1. KB Query API Test
echo -e "\n${YELLOW}1. Testing KB Query API...${NC}"
...
# 2. Upload URL API Test
echo -e "\n${YELLOW}2. Testing Upload URL API...${NC}"
...
# 3. KB Sync API Test
echo -e "\n${YELLOW}3. Testing KB Sync API...${NC}"
...
SYNC_RESPONSE=$(curl -s -X POST \
  ...
  -H "x-api-key: ${API_KEY}" \
  -d @kb_sync.json \
  ${SECURE_API_URL}/kb/sync || echo "Request failed")

echo -e "${GREEN}✓ KB Sync API Response:${NC}"
echo "$SYNC_RESPONSE" | jq . 2>/dev/null || echo "$SYNC_RESPONSE"

# 4. KB Ingestion Status API Test
echo -e "\n${YELLOW}4. Testing KB Ingestion Status API...${NC}"
...
STATUS_RESPONSE=$(curl -s -X POST \
  ...
  ${SECURE_API_URL}/kb/ingestion-status || echo "Request failed")

echo -e "${GREEN}✓ KB Ingestion Status API Response:${NC}"
echo "$STATUS_RESPONSE" | jq . 2>/dev/null || echo "$STATUS_RESPONSE"

# 5. Chat API Test
echo -e "\n${YELLOW}5. Testing Chat API...${NC}"
...
CHAT_RESPONSE=$(curl -s -X POST \
  ...
  -H "x-api-key: ${API_KEY}" \
  -d @chat_query.json \
  ${SECURE_API_URL}/chat || echo "Request failed")
